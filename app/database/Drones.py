...
class Drones(SafeDocument, DroneModel):
    ...
    async def findByFirstLaunch(date_time: datetime):
        ...
        response_dict["traditional_find_many"] = await Drones.find_many(
            {"first_launch": {"$gte": date_time}}
        ).to_list()
        response_dict["using_GTE_operator"] = await Drones.find_many(GTE(Drones.first_launch, date_time)).to_list()
        response_dict["using_find_one"] = await Drones.find_one(Drones.first_launch == date_time)
        response_dict["using_all_then_filter"] = [*filter(
            lambda item:item.first_launch == date_time,
            await Drones.all().to_list())]
